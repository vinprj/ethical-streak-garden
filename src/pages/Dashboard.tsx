...
const Dashboard: React.FC = () => {
  ...
  const recentlyCompletedHabits = activeHabits.filter(habit => habit.completedDates.includes(today));
  ...
  return <AppLayout>
      <div className={`flex flex-col gap-8 transition-all duration-500 ${isLoaded ? 'opacity-100' : 'opacity-0'}`}>
        ...
        <section className="transition-all duration-500" style={{
        transitionDelay: "100ms"
      }}>
          <div className="flex justify-between items-center mb-6 flex-wrap gap-4">
            <div>
              <h1 className="text-3xl font-bold">Welcome to Routine Garden</h1>
          ...
          {activeHabits.length > 0 && <div className="mb-6">
              <UserStats />
            </div>}
        </section>
        ...
        transitionDelay: "150ms"
          ...
          {activeHabits.length > 0 ? <div className="grid gap-4 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3">
              {recentlyCompletedHabits.length > 0 ? <>
                  ...
                  {recentlyCompletedHabits.slice(0, 3).map(habit => <HabitCard key={habit.id} habit={habit} />)}
        ...
        {/* Badges section */}
        {unlockedBadges.length > 0 && <section className="transition-all duration-500" style={{
        ...
        transitionDelay: "250ms"
          ...
          {activeHabits.length > 0 ? <div className="grid gap-6">
              {healthHabits.length > 0 && <div>
                  <h3 className="font-medium mb-3">Health</h3>
                  <div className="grid gap-4 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3">
                    {healthHabits.slice(0, 3).map(habit => <HabitCard key={habit.id} habit={habit} />)}
                  </div>
                </div>}
              
              {mindfulnessHabits.length > 0 && <div>
                  <h3 className="font-medium mb-3">Mindfulness</h3>
                  <div className="grid gap-4 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3">
                    {mindfulnessHabits.slice(0, 3).map(habit => <HabitCard key={habit.id} habit={habit} />)}
                  </div>
                </div>}
              
              {productivityHabits.length > 0 && <div>
                  <h3 className="font-medium mb-3">Productivity</h3>
                  <div className="grid gap-4 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3">
                    {productivityHabits.slice(0, 3).map(habit => <HabitCard key={habit.id} habit={habit} />)}
                  </div>
                </div>}
            </div> : null}
        </section>
